...
class JetHTHybrid3DTriggerEfficienciesProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        try:
            ...
            Txbb_scores = ak.nan_to_num(ak.fill_none(ak.pad_none(events2.FatJet.particleNetMD_Xbb / (events2.FatJet.particleNetMD_QCD + events2.FatJet.particleNetMD_Xbb), 2, axis=1), 0), 0)
            ...
            dR = 0.8
            no_both_overlap = ak.flatten((events2.FatJet[bb_mask].delta_r(events2.FatJetAK15[:, 0]) > dR) + (ak.fill_none(events2.FatJet[bb_mask].delta_r(ak.pad_none(events2.FatJetAK15, 2, axis=1)[:, 1]) > dR, False)))

            # only use events which have two non-overlapping fat jets
            events2 = events2[no_both_overlap]
            bb_mask = bb_mask[no_both_overlap]

            Th4q_scores = ak.nan_to_num(ak.fill_none(ak.pad_none(events2.FatJetAK15.ParticleNet_probHqqqq / (events2.FatJetAK15.ParticleNet_probHqqqq +
            ...
            # passing all HLT triggers
            bbVV_triggered = np.any(np.array([events2.HLT[trigger] for trigger in self.HLTs[2017]]), axis=0)
