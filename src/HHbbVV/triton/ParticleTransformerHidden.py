...
class PairEmbed(nn.Module):
    def __init__(
        ...
        self.pairwise_lv_fts = partial(
            pairwise_lv_fts, num_outputs=input_dim, eps=eps, for_onnx=for_onnx
        )
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if not self.for_onnx:
                ...
                x = self.pairwise_lv_fts(x.unsqueeze(-1), x.unsqueeze(-2)).view(
...
class ParticleTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pair_embed = (
            PairEmbed(
                pair_input_dim, pair_embed_dims + [cfg_block["num_heads"]], for_onnx=for_inference
            )
    ...
    def forward(self, x, v=None, mask=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            if v is not None and self.pair_embed is not None:
                attn_mask = self.pair_embed(v).view(
                    -1, v.size(-1), v.size(-1)
                )  # (N*num_heads, P, P)
