...
def getVBFGenMatchCount(self, events: ak.Array,jets: ak.Array,bbJet,VVJet,base_mask,bbJet_Txbb):
    ...
    for R1 in np.arange(0.5, 2, 0.1):
        for R2 in np.arange(0.5, 2, 0.1):
            ...
            mask_VVJet = (VVJet.pt >= 300)  & (np.abs(VVJet.eta) < 4.7) & ((VVJet.M >= 50) | (VVJet.M <= 250))
            mask_bbJet = (bbJet.pt >= 300) & (bbJet.M > 50) & (np.abs(VVJet.eta) < 4.7) & (bbJet_Txbb > 0.8)
            m2 = mask_VVJet & mask_bbJet
            mask_sorted_pt = num_sorted_pt > 1
            

            # Combine masks with logical 'and' operation
            total_mask = m2 & mask_sorted_pt # this is comendd out for og events debug applying this since the sizes ndont match

            # Apply the mask to filter num_per_event
            #logging.warning(f'{len(total_mask)} {len(num_per_event)}')
            num_per_event = num_per_event[total_mask]#num_per_event = np.where(np.isnan(num_per_event.astype(float)), 0, num_per_event).astype(int) 
            ...
            counts = np.bincount(num_per_event, minlength=3)  # Ensure counts for all [0, 1, 2] even if some might be missing
            #logging.warning(f"[{counts[0]}, {counts[1]}, {counts[2]}, {len(num_per_event)}, {R1:.1f}, {R2:.1f}]")
            
            result = [(counts[1]+counts[2])/ak.sum(m2), R1, R2] # normalize count by number of events passing ak8 jet selections
...
def getVBFGenPtvsR2data(self, events: ak.Array,jets: ak.Array,bbJet,VVJet,base_mask):
    ...
    for i in range(len(VVJet)):  # loop over the events
        ...
        for R2 in np.arange(0.6, 2, 0.1):
            ...
            fatjet_overlap_mask = (np.abs(jets[i].delta_r(bbJet[i])) > R1) & (np.abs(jets[i].delta_r(VVJet[i])) > R2)
            ...
            successfulMatch = int(((delta_R_11 < 0.4) and (delta_R_22 < 0.4)) or ((delta_R_12 < 0.4) and (delta_R_21 < 0.4)))
...
def getTruthDeltaRDistribution(self, events,vbfJets,bbJet,VVJet,bbJet_Txbb):
    ...
    drj2bb = genjet2.delta_r(bbJet) # I believe this will have some empty values hopefully still len 9600
    ...
    result = [[drj1bb_val, drj2bb_val, drj1VV_val, drj2VV_val] 
            for drj1bb_val, drj2bb_val, drj1VV_val, drj2VV_val 
            in zip(drj1bb_filtered, drj2bb_filtered, drj1VV_filtered, drj2VV_filtered)]
    ...
    if True:
        ...
        with open(output_file, "a") as f:
            for pair in result:
                cleaned_pair = [float(val[0]) for val in pair]  # Convert each awkward array to a simple float
...
def getVBFGenvsgenHH2ddata(self, events: ak.Array,bbJet_Txbb):
    ...
    for i in range(len(HVV)):  # loop over the events
        ...
        # Store the quantities in a list
        result = [reco_1.pt,reco_2.pt, delta_R_11,delta_R_12,delta_R_21, delta_R_22,bbJet_Txbb[i]]
        logging.warning(f'{result} , {delta_R_11} {delta_R_21} {delta_R_12} {delta_R_22} ')
